...
def plot_1d_data(spec_data_file, scans, x, y) -> None:
    ...
    for idx, y_ in enumerate(y):
        ...
        for j, s_n in enumerate(scans):
            ...
            ax.plot(scan.data[x], scan.data[y_], label=str(s_n), color=colors[j], alpha=0.5)
    
    handles, labels = fig.axes[0].get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper right")
    fig.show()

def plot_3d_isosurface(array: np.ndarray, contours=10, alpha=0.2, cmap="jet") -> None:
...
def plot_fwhm_cent_auc(spec_data_file, scans, x, y, z) -> None:
    ...
    for idx, s_n in enumerate(scans):
        ...
        params = model.guess(
            np.array(scan.data[y]),
            x=np.array(scan.data[x])
        )
        ...
        ax_raw.plot(scan.data[x], scan.data[y], label=str(round(avg_z, 6)), color=colors[idx], alpha=0.5)
        ax_fit.plot(scan.data[x], out.best_fit, label=str(round(avg_z, 6)), color=colors[idx], alpha=0.5)
...
def plot_strain_series(spec_data_file, scans, wavelength, offset, x_val):
    ...
    for idx, s_n in enumerate(scans):
        ...
        if x_val == "force":
            strain.append(cap_to_force(scan.data["cap"][0], scan.data["Temp_sam"][0]))
        elif x_val == "rbv":
...
def plot_2d_orthogonal_slice(data, coords=None, x=None, y=None, z=None, scale="linear", contour=False, 
    cmap="jet", axes=["x", "y", "z"]):
    if coords is None:
    ...
    if type(x) == int or type(x) == float:
        ...
        y_range = [np.searchsorted(coords[1], y[0]), np.searchsorted(coords[1], y[1])]
        z_range = [np.searchsorted(coords[2], z[0]), np.searchsorted(coords[2], z[1])]

        orth_slice = data[idx,y_range[0]:y_range[1],z_range[0]:z_range[1]].T
        ...
        x_range = [np.searchsorted(coords[0], x[0]), np.searchsorted(coords[0], x[1])]
        z_range = [np.searchsorted(coords[2], z[0]), np.searchsorted(coords[2], z[1])]

        orth_slice = data[x_range[0]:x_range[1],idx,z_range[0]:z_range[1]].T
        ...
        orth_slice = data[x_range[0]:x_range[1],y_range[0]:y_range[1], idx].T
    ...
    if contour:
        ...
        plt.contourf(orth_slice, norm=scale, extent=extent, origin="lower", cmap=cmap, aspect="auto", levels=levels)
        plt.xlabel(x_axis_lbl)
        plt.ylabel(y_axis_lbl)
        plt.show()
    else:
        plt.imshow(orth_slice, norm=scale, extent=extent, origin="lower", cmap=cmap, aspect="auto", interpolation="none")
        plt.xlabel(x_axis_lbl)
        plt.ylabel(y_axis_lbl)
        plt.show()
    
def plot_1d_line_cuts(spec_data_file, scans, point_1, point_2, z_val, instr_config_path, det_config_path, radius=5, log_scale=False, plot_3d=True):
    line_cuts = []
    z_values = []
    ...
    set_bounds = None
    for idx, s_n in enumerate(scans):
        scan_image_directory = "/home/beams/USER6IDB/Data/run23_2_lab2/staff/EuAl4/images/EuAl4_001_1/S" + str(s_n)
        scan = spec_data_file.getScan(s_n)
        ...
        line_cut_data, lcc = get_1d_line_cut(rsm_data, rsm_data_coords, point_1, point_2, radius)
        ...
        line_cut_bounds = [[lcc_h_min, lcc_h_max],
            [lcc_k_min, lcc_k_max],
            [lcc_l_min, lcc_l_max]]
        ...
        if plot_3d:
            ax.plot(range(line_cut_data.shape[0]), line_cut_data, scan.data[z_val][0], zdir="y", color=colors[idx], alpha=0.75)
        else:
            if line_cut_bounds[0][0] != line_cut_bounds[0][-1]:    
                ax.plot(np.linspace(line_cut_bounds[0][0], line_cut_bounds[0][-1], line_cut_data.shape[0]), line_cut_data, label=str(round(scan.data[z_val][0], 5)), color=colors[idx], alpha=0.75)
            elif line_cut_bounds[1][0] != line_cut_coords[1][-1]:    
                ax.plot(np.linspace(line_cut_bounds[1][0], line_cut_bounds[1][-1], line_cut_data.shape[0]), line_cut_data, label=str(round(scan.data[z_val][0], 5)), color=colors[idx], alpha=0.75)
            else:
                ax.plot(np.linspace(line_cut_bounds[2][0], line_cut_bounds[2][-1], line_cut_data.shape[0]), line_cut_data, label=str(round(scan.data[z_val][0], 5)), color=colors[idx], alpha=0.75)
...
def get_raw_image_data(spec_scan, instr_config_path, image_dir) -> np.ndarray:
    ...
    for i, file in enumerate(sorted(os.listdir(image_dir))):
        if file.endswith("tif") and "alignment" not in file:
            ...
            monitor_norm_factor = spec_scan.data["Ion_Ch_2"][i] * instrument_reader.getMonitorScaleFactor()
            filter_norm_factor = spec_scan.data["transm"][i] * instrument_reader.getFilterScaleFactor()

            norm_factor = monitor_norm_factor * filter_norm_factor
            norm_image_array = image_array * norm_factor

            raw_data.append(norm_image_array)
...
def get_1d_line_cut(data, coords, point_1, point_2, radius):
    ...
    points_to_average = np.repeat(points, offsets_array.shape[0], axis=0) + np.tile(offsets_array, (points.shape[0], 1))
    ...
    for idx, pt in enumerate(points_to_average):
        valid = np.all((points_to_average[idx] >= 0) & (points_to_average[idx] < data.shape), axis=1)
        valid_points = points_to_average[idx][valid]
        smoothed_data_point = np.mean(data[valid_points[:, 0], valid_points[:, 1], valid_points[:, 2]])
        smoothed_line_cut_data.append(smoothed_data_point)

    line_cut_data = np.array(smoothed_line_cut_data)
    line_cut_coords = [coords[0][valid_points[:, 0]], coords[1][valid_points[:, 1]], coords[2][valid_points[:, 2]]]
...
def create_rsm(spec_scan: spec.SpecDataFileScan, instr_config_path: str, det_config_path: str) -> np.ndarray:
...
def cap_to_force(cap, T = 290):
    ...
    p = np.array([7.53621149e-10, 1.13610411e-06, 2.65618549e-04, 1.58396569e+00])
